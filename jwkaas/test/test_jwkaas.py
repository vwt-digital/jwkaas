...
class TestJWKaas(unittest.TestCase):
    ...
    def test_decode_valid_token(self):
        ...
        token = jwt.encode(
            token_body, self.private_key, algorithm="RS256", headers={"kid": "test-key"}
        )
    ...
    def test_decode_unexpected_issuer_token(self):
        ...
        logging.info("Token info: {}".format(info))
        self.assertEqual(
            info, None, "Token with unexpected issuer should not be decoded"
        )
    ...
    def test_decode_hmac_token(self):
        ...
        token = jwt.encode(
            token_body, self.private_key, algorithm="HS256", headers={"kid": "test-key"}
        )
        info = self.my_jwkaas.get_token_info(token)
        logging.info("Token info: {}".format(info))
        self.assertEqual(
    ...
    def test_decode_tampered_body_token(self):
        ...
        new_body = base64.b64encode(
            json.dumps(tampered_token_body).encode("utf-8")
        ).decode("utf-8")
    ...
    def test_connexion_decode_invalid_token_with_scp_and_roles(self):
        ...
        info = self.my_jwkaas.get_connexion_token_info(token)
        self.assertEqual(
            info, None, "Invalid token should not be decoded by connexion_decode"
        )
